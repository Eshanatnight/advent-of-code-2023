    ...
    let dv = input
        ...
        .map(|l| {
            ...
            let conns = l[5..]
                ...
                .map(|s| find_index(&mut name_indices, s))
                .collect::<Vec<_>>();
            (index, conns)
        })
        .collect::<Vec<_>>();
...
fn path(diagram: &[Vec<usize>], q: &mut VecDeque<usize>, a: usize, b: usize) -> Vec<usize> {
    let dl = diagram.len();
...
pub fn solve(input: &str) -> Option<usize> {
    ...
    for i in 1.. {
        ...
        let spath = path(&diagram, &mut dq, a, b);
        if spath.is_empty() {
            continue;
        }
