        ...
        std::io::BufReader::new(std::io::stdin())
            ...
            .map(|line| {
                ...
                while offset < 1 + result.len() && re.is_match_at(&result, offset) {
                    let re = Regex::new(&format!(
                        r#"^(.{{{}}}.*?){}(.*)$"#,
                        offset, r#"(one|two|three|four|five|six|seven|eight|nine)"#
                    ))
                    .unwrap();
            ...
            .map(|line| {
                ...
                match (captures.name("first"), captures.name("last")) {
                    ...
                    (Some(first), None) => format!("{}{}", first.as_str(), first.as_str())
                        .parse::<i32>()
                        .unwrap(),
                    (Some(first), Some(last)) => format!("{}{}", first.as_str(), last.as_str())
                        .parse::<i32>()
                        .unwrap(),
                }
            })
