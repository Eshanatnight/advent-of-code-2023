...
auto is_fixable_mirror_row(const std::vector<std::string>& patterns, int mirror_row, int test_row, bool& fixed) -> bool {
    int32_t distance = std::abs(mirror_row - test_row);

    if (test_row < 0 || test_row + ((distance * 2) - 1) >= patterns.size()) {
        return true;
    } else {
        int64_t differences = are_different_rows(patterns, test_row, test_row + ((distance * 2) - 1));
        if (differences == 1) {
            fixed = true;
...
auto is_fixable_mirror_cols(const std::vector<std::string>& patterns, int mirror_col, int test_col, bool& fixed) -> bool {
